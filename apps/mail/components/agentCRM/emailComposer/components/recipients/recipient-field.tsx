...
function getInitials(str: string): string {
  ...
  // fallback: first 2 chars or first char before @
...
function RecipientChip({ tag, onRemove, disabled }: { tag: EmailTag; onRemove: () => void; disabled?: boolean }) {
  ...
  return (
    <div
      className="group inline-flex items-center gap-1.5 rounded-full bg-zinc-100 hover:bg-zinc-200/80 border border-zinc-200 transition-colors duration-150 pl-[3px] pr-2 h-[26px] max-w-[240px] flex-shrink-0"
      title={tooltip}
    >
      {/* Avatar */}
      <div
      ...
      {/* Name */}
      <span className="text-[12px] font-medium text-zinc-700 truncate leading-none">
        {displayName}
      </span>
      ...
      {!disabled && (
        <button
          ...
          className="flex-shrink-0 w-4 h-4 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 hover:bg-zinc-300 transition-all duration-100 ml-0.5 cursor-pointer"
...
export function RecipientField({
  ...
  return (
    <div className="divide-y divide-gray-100">
      {/* To row */}
      <div className="relative flex items-start min-h-[42px]">
        <span className="w-8 flex-shrink-0 text-[11px] font-semibold text-gray-400 uppercase tracking-wider pt-[13px] pl-4 select-none">
          To
        </span>
        <div className="flex-1 flex flex-wrap items-center gap-1.5 py-2 pr-2 pl-1 min-h-[42px]">
          {toTags.map((tag) => (
            <RecipientChip
              key={tag.id}
              tag={tag}
          ...
          <Input
            ...
            disabled={!!replyTo}
            className="border-0 bg-transparent focus-visible:ring-0 p-0 h-[26px] flex-1 min-w-[120px] text-[13px] text-gray-700 placeholder:text-gray-300"
          />
        </div>
        <div className="flex items-center gap-0.5 pr-3 pt-[9px] flex-shrink-0">
          {!showCc && (
            <button
              className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider hover:text-gray-600 transition-colors px-1.5 py-1 rounded hover:bg-gray-100"
      ...
      {/* Cc row */}
      {showCc && (
        <div className="flex items-start min-h-[42px]">
          <span className="w-8 flex-shrink-0 text-[11px] font-semibold text-gray-400 uppercase tracking-wider pt-[13px] pl-4 select-none">
            Cc
          </span>
          <div className="flex-1 flex flex-wrap items-center gap-1.5 py-2 pr-2 pl-1 min-h-[42px]">
          ...
          <button
            className="p-2 mt-1.5 mr-1 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded transition-colors flex-shrink-0"
